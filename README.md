...
Форк репозитория создан, создан dockerfile (Dockerfile.python), образ собирается, при этом файл requirements.txt копируется в образ для дальнейшей настройки окружения.
...
Приложение db создается в соответствии с заданными параметрами, переменные для создания пароля root, БД и данных пользователя передаются через файл .env.
...
Результат подключения к контейнеру с БД и выполнения запроса представлен на скриншоте:
